...
```ingest-unclassified source=vol08.pdf page=131
Item   If the asset’s        This is the result for the     This is the result for the
       *market value at      asset:                        one or more liabilities:
       the joining time:
1      is less than or       its *tax cost setting amount  only the difference (if
       equal to the          is that market value (and     any) is taken into account
       available amount      the asset is _not_ taken into under
                             account under                 subsection 705-70(1) for
                             paragraph 705-35(1)(c))       the one or more liabilities
```

**Treatment of linked assets and liabilities: single reset cost base asset case**

```ingest-unclassified source=vol08.pdf page=132
Item   If the asset’s        This is the result for the     This is the result for the
       *market value at      asset:                        one or more liabilities:
       the joining time:
2      is greater than the   its *tax cost setting amount  the one or more liabilities
       available amount      is:                           are _not_ taken into
                             (a) the available amount;     account under
                                 plus                      subsection 705-70(1)
                             (b) the amount worked out
                                 for the asset under
                                 section 705-35 on the
                                 basis that the asset’s
                                 *market value is
                                 reduced by the
                                 available amount
```
