---
act: "ITAA 1997"
part: "3-10"
part_title: "Financial transactions"
division: "250"
division_title: "Assets put to tax preferred use"
subdivision: "250-E"
subdivision_title: "Taxation of deemed loan"
section: "250-235"
section_title: "Overview of the accruals method"
compilation_no: 266
compilation_date: "2026-07-01"
source_pdf: "vol05.pdf"
---
# 250-235  Overview of the accruals method

If the accruals method applies to a gain or loss you make from the *financial arrangement:

> <a id="s250-235-a"></a>
> **(a)** you use section 250-240 to work out the period over which the gain or loss is to be spread; and

> <a id="s250-235-b"></a>
> **(b)** you use section 250-245 to work out how to allocate the gain or loss to particular intervals within the period over which the gain or loss is to be spread; and

> <a id="s250-235-c"></a>
> **(c)** if an interval to which part of the gain or loss is allocated straddles 2 income years, you use section 250-250 to work

out how to allocate that part of the gain or loss allocated between those 2 income years.

---
*Last updated: 2026-07-01 (Compilation 266)*
