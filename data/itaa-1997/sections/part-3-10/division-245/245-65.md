...
subdivision: "245-C"
subdivision_title: "Calculation of gross forgiven amount of a debt"
section: "245-65"
...
```ingest-unclassified source=vol05.pdf page=521
Amount offset against value of debt

Item   Column 1                                 Column 2
       In this case:                            the amount offset is:

1      the debt is a *moneylending debt,        the sum of:
...
```ingest-unclassified source=vol05.pdf page=522
6      the debt is *forgiven by subscribing     the amount worked out using the
```

```ingest-unclassified source=vol05.pdf page=523
Amount offset against value of debt

Item   Column 1                                 Column 2
       In this case:                            the amount offset is:

       for *shares in a company as              formula in subsection (3).
...
```ingest-formula source=vol05.pdf page=523
       Amount applied
                            ´  Market value of shares subscribed for
     Amount subscribed
```

where: amount applied means the amount applied by the company as mentioned in section 245-37. amount subscribed means the amount subscribed as mentioned in section 245-37. market value of shares subscribed for means the *market value of all the shares in the company that were subscribed for as

mentioned in section 245-37, immediately after those shares were issued. Working out the gross forgiven amount
