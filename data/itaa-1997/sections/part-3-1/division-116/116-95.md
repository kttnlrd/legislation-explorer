...
```ingest-formula source=vol03.pdf page=469
                                   CFC asset profit
Distributable profit amount   ´
                                  Total asset profits
```

where: total asset profits is the sum of the profits that the CFC would have made if all its assets were *disposed of at the residency change time for their *market values (ignoring disposals that would not result in a profit).
...
```ingest-formula source=vol03.pdf page=470
Distributable profit               CFC asset loss
                             ´
reduction amount                 Total asset losses
```

where: total asset losses is the sum of the losses that the CFC would have made if all its assets were *disposed of at the residency change time for their *market values (ignoring disposals that would not result in a loss).
